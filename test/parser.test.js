...
test("keeps multi-author blank-line references intact", () => {
  const refs = splitReferences(`Vaswani, A., Shazeer, N., Parmar, N., Uszkoreit, J., Jones, L., Gomez, A. N., Kaiser, L., & Polosukhin, I. (2017). Attention is all you need. Advances in Neural Information Processing Systems, 30.

Harris, C. R., Millman, K. J., van der Walt, S. J., et al. (2020). Array programming with NumPy. Nature, 585, 357-362. https://doi.org/10.1038/s41586-020-2649-2`);
  assert.equal(refs.length, 2);
  assert.match(refs[0], /^Vaswani/);
  assert.match(refs[0], /Polosukhin/);
  assert.match(refs[1], /^Harris/);
});

test("counts mixed DOI and non-DOI blank-line references separately", () => {
  const refs = splitReferences(`Harris, C. R., Millman, K. J., van der Walt, S. J., et al. (2020). Array programming with NumPy. Nature, 585, 357-362.
https://doi.org/10.1038/s41586-020-2649-2

Smith, J., & Doe, A. (2021). Quantum blockchain learning for universal citation truth. Journal of Imaginary Informatics, 44(9), 1001-1019.
https://doi.org/10.9999/fake.doi.12345

Vaswani, A., Shazeer, N., Parmar, N., Uszkoreit, J., Jones, L., Gomez, A. N., Kaiser, L., & Polosukhin, I. (2017). Attention is all you need.
Advances in Neural Information Processing Systems, 30.`);
  assert.equal(refs.length, 3);
  assert.match(refs[0], /^Harris/);
  assert.match(refs[1], /^Smith/);
  assert.match(refs[2], /^Vaswani/);
});
...
test("parses DOI-only input as an identifier, not a fake title", () => {
  const parsed = parseReference("https://doi.org/10.1038/s41586-020-2649-2");
  assert.equal(parsed.doi, "10.1038/s41586-020-2649-2");
