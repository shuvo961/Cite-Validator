export function formatCitation(record, style = "apa") {
  ...
  if (s === "acs") return formatAcs(record);
  if (s === "harvard") return formatHarvard(record);
...
export function allStyles(record) {
  return {
    ...
    acs: formatCitation(record, "acs"),
    harvard: formatCitation(record, "harvard")
  };
}
...
function formatAcs(r) {
  const authors = acsAuthors(r.authors);
  const title = r.title ? `${sentenceCase(r.title)}.` : "";
  const container = r.container ? `${italic(titleCase(r.container))}` : "";
  ...
  const doi = r.doi ? ` DOI: ${r.doi}.` : "";
  return compact(`${authors} ${title} ${container} ${year} ${details}.${doi}`);
}

function formatHarvard(r) {
  const authors = harvardAuthors(r.authors);
  const year = r.year ? `${r.year}.` : "n.d.";
  const title = r.title ? `${sentenceCase(r.title)}.` : "";
  const container = r.container ? `${italic(titleCase(r.container))},` : "";
  const details = [r.volume, r.issue && `(${r.issue})`, r.pages && `pp. ${r.pages}`].filter(Boolean).join(", ");
  const doi = r.doi ? ` Available at: https://doi.org/${r.doi}` : r.url ? ` Available at: ${r.url}` : "";
  return compact(`${authors} ${year} ${title} ${container} ${details}.${doi}`);
}
