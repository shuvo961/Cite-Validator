...
- For production, move saved validation jobs to PostgreSQL. See `PRODUCTION.md`.
- Add Redis-backed rate limiting/cache before significant public traffic. The MVP includes in-memory rate limiting for a single Node instance.
- The app includes `/privacy`, `/terms`, and `/security`; review their wording before public launch.
- Do not commit `.env` or SQLite database files. This repo now ignores them.
- Configure Google OAuth with redirect URL: `https://your-domain.com/auth/google/callback`.
- Confirm the hidden admin login URL `/ownershuvo` is not linked publicly.
- Rotate any local test admin password before launch.

## Quick Public Launch Checklist

- Set `CONTACT_EMAIL`.
- Set `APP_BASE_URL` to your real domain.
- Set Google OAuth credentials if login should work publicly.
- Confirm `node --test` passes.
- Deploy on Render, Railway, Fly.io, or a VPS.
- Point your domain DNS to the hosting provider.
- Test `/`, `/validate`, `/converter`, `/doi-checker`, `/fake-citation-detector`, `/privacy`, `/terms`, `/security`, `/sitemap.xml`, and `/api/health`.
