...
export function parseReference(reference) {
  ...
  const identifierOnly = isIdentifierOnlyReference(text, { doi, isbn, issn, arxiv, pmid });
  const year = extractYear(text);
  const authors = identifierOnly ? [] : extractAuthors(text);
  const title = identifierOnly ? "" : extractTitle(text, authors, year);
  const container = identifierOnly ? "" : extractContainer(text, title);
...
function isIdentifierOnlyReference(text, identifiers) {
  ...
  return values.some((value) => compact === value || compact === `pmid:${value}` || compact === `arxiv:${value}`);
}
...
function looksLikeContainer(value) {
  return /\b(journal|proceedings|conference|transactions|letters|review|annals|science|nature|lancet|jama|ieee|acm|press|publisher|university|arxiv|advances in neural information processing systems|neurips|nips)\b/i.test(value);
}

function guessStyle(text) {
  if (/^\s*\[\d+\]/.test(text)) return "IEEE";
  if (/^\s*\d+\.\s+[A-Z]/.test(text) || /\bJ\.\s+[A-Z][A-Za-z.]+\b/.test(text)) return "ACS";
  if (/\(\d{4}[a-z]?\)/.test(text)) return "APA";
  if (/\b[A-Z][A-Za-z-]+,\s+[A-Z]\./.test(text) && /\bpp\.\s*\d+/i.test(text)) return "Harvard";
