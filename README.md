...
- Shows a corrected reference in APA, IEEE, MLA, Chicago, Vancouver, ACS, or Harvard style.
- Exports CSV, PDF, DOC, and BibTeX reports.
- Includes a public product landing page, citation converter, SEO guide pages, Google OAuth foundation, user dashboard, saved validation history, hidden owner/admin access, production readiness checks, and admin monitoring portal.
...
No npm packages are required. Node 24+ is enough for the local MVP.
...
- `/login` public Google-only login. If Google OAuth credentials are absent, the app explains that production login is not configured.
- `/dashboard.html` logged-in full workspace.
- `/ownershuvo` hidden owner/admin portal for users, usage, source health, readiness, and feedback.
...
- `GET /api/health`
  - Returns app health and production readiness.
- `GET /api/system/readiness`
  - Returns deployment readiness checks for OAuth, session secret, base URL, database, queue, email, and HTTPS.
- `POST /api/validate`
  - Body: `{ "referencesText": "...", "style": "apa" }`
  - Returns a validation job with per-reference results. Large batches above `SYNC_VALIDATION_LIMIT` return a queued job that can be polled via `GET /api/jobs/:id`.
- `GET /api/jobs/:id`
  - Returns a saved job.
...
For public users, `/login` shows Google sign-in only. The hidden owner/admin portal is `/ownershuvo` and is restricted to `shovon961@gmail.com`.
...
Admin access is hard-locked to `shovon961@gmail.com` in the app code and environment example.
